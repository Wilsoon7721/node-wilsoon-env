...
Arrow keys move, Enter picks, or press the number. Each question says where its value comes from, and the sections are labelled so you can see when it stops asking _where the files live_ and starts asking _how this machine proves it may fetch them_ - two different questions that want two different credentials.

It never asks for a secret. The config it writes is meant to be committed, so anything that would compromise the store if published is named as an environment variable for you to set instead. Before it generates a key it checks the store actually answers, so a wrong table or a missing credential costs you nothing - and if you got something wrong, it offers to keep your answers so the next run does not ask again.
...
Saved stores live beside your credentials in your user config directory, never in a repository.

### Scripting it instead

Everything the wizard asks can be passed as a flag, but only behind `--unattended`:

```bash
npx @wilsoon/env setup --unattended --provider s3 \
  --bucket my-secrets --endpoint https://xyz.r2.cloudflarestorage.com
```

Without that word the flags are refused, and `--help` stays short. That is deliberate: a first-time reader should meet six options and a note that setup will ask, not nineteen provider settings. It also means a scripted run says so out loud rather than being inferred from whether a terminal happened to be attached.

For the full list, grouped per provider with defaults and the environment variable each store reads its credentials from:

```bash
npx @wilsoon/env setup --unattended --help
```

Set `WILSOON_ENV_PASSPHRASE` and nothing has to be typed at all.
...
### What row level security gets you
...
```bash
npx @wilsoon/env setup --provider supabase --url https://xyz.supabase.co --anon-key "$SUPABASE_ANON_KEY" --table blobs --schema my_schema --auth oidc --issuer https://id.example.com --client-id my-client
```
...
`setup` cannot write to it until you are signed in, and `login` reads the issuer out of the config `setup` has not written yet. Break it by naming the issuer on the command line - no config needed:
...
**Device grant ([RFC 8628](https://www.rfc-editor.org/rfc/rfc8628)) is preferred.** Used whenever the issuer advertises `device_authorization_endpoint`. You get a short code and a URL:
...
Without an TTY, the CLI fails in milliseconds with instructions rather than blocking on a prompt nobody can answer.
