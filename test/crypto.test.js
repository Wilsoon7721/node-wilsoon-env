...
describe('kdf', () => {
    ...
    it('refuses an empty passphrase', async () => {
        await expect(deriveKey('', randomBytes(16), FAST_KDF)).rejects.toThrow(/empty passphrase/i);
    });
...
describe('public key encoding', () => {
    ...
    it('rejects a key without the prefix', () => {
        expect(() => decodePublic('abcdef')).toThrow(/wenv1/);
    });
});
...
describe('envelope', () => {
    ...
    it('needs at least one recipient', () => {
        expect(() => seal({ plaintext: ENV_FILE, recipients: [], ...CONTEXT })).toThrow(/at least one recipient/i);
    });
